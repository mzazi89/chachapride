 ...
 */
export default function InstallBanner({ appName = 'chachapride' }) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[9999] w-[calc(100%-2rem)] max-w-md animate-slide-up">
      <div className="flex items-center gap-3 bg-slate-900/95 backdrop-blur border border-slate-700 text-white rounded-2xl shadow-2xl px-4 py-3">
        <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-600 to-green-500 flex items-center justify-center text-white text-lg shrink-0">
          <FaDownload />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-bold text-sm leading-tight">Install {appName} app</p>
          <p className="text-xs text-slate-400">Open it anytime, even offline</p>
        </div>
        <button
          onClick={handleInstall}
          className="px-4 py-2 rounded-full bg-gradient-to-r from-blue-600 to-green-500 text-white text-sm font-bold hover:brightness-110 transition shrink-0"
        >
          Install
        </button>
        <button
